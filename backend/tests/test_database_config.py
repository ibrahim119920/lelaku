...
def test_transaction_pooler_can_disable_prepared_statements(
    monkeypatch: pytest.MonkeyPatch,
    value: str,
):
    monkeypatch.setenv("DB_DISABLE_PREPARED_STATEMENTS", value)

    assert _get_psycopg_connect_args() == {
        "connect_timeout": 10,
        "prepare_threshold": None,
    }


def test_prepared_statements_remain_enabled_by_default(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("DB_DISABLE_PREPARED_STATEMENTS", raising=False)

    assert _get_psycopg_connect_args() == {"connect_timeout": 10}
