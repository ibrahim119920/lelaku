...
def _get_psycopg_connect_args() -> dict[str, object]:
    connect_args: dict[str, object] = {"connect_timeout": 10}
    disable_prepared = os.getenv("DB_DISABLE_PREPARED_STATEMENTS", "false").strip().lower()
    if disable_prepared in {"1", "true", "yes", "on"}:
