...
2. Untuk backend FastAPI lokal yang berjalan lama, pilih **Session pooler** jika
   ...
   Jika harus memakai **Transaction pooler**, set
   `DB_DISABLE_PREPARED_STATEMENTS=true` di `backend/.env`; psycopg akan
   menonaktifkan prepared statements agar koneksi kompatibel dengan pooler itu.
   Untuk direct connection atau Session pooler, nilai default `false` dapat
   dipertahankan.
...
## Tambahan skema API trip dan matching

Pada branch integrasi LL-05 dengan 07-08, migrasi Bayu dilanjutkan dari revisi
LL-05 20260923_0001; migrasi placeholder auth Bayu tidak dipakai karena
membuat ulang tabel users, driver_profiles, dan user_sessions.
Revisi berikutnya menambahkan vehicles, trips, trip_matches,
ride_requests, trip_members, messages, dan ratings secara bertahap.

Kolom users.user_id tetap TEXT sesuai migrasi LL-05. Foreign key dari tabel
trip, kendaraan, chat, dan rating ke user juga disimpan sebagai TEXT; model
backend memetakan nilai UUID tersebut ke uuid.UUID saat dipakai di Python.
Dengan demikian, integrasi tidak mengubah tipe kolom user yang sudah ada.

Penambahan migrasi ini di branch integrasi tidak dengan sendirinya mengubah
Supabase. Sebelum menjalankan alembic upgrade head pada database mana pun,
periksa revisi Alembic yang sudah tercatat dan pastikan migration chain-nya
sesuai dengan revisi LL-05.
